		...
		case 'SaveOrUpdate':
			...
			if(move_uploaded_file($imagen, "../Files/Articulo/".$ruta)){
				...
				if(empty($_POST["txtIdArticulo"])){
					...
					if($objArticulo->Registrar($idcategoria, $idunidad_medida, $nombre, $descripcion, $marca, "Files/Articulo/".$ruta)){
					...
					if($objArticulo->Modificar($idarticulo, $idcategoria, $idunidad_medida, $nombre, $descripcion, $marca, "Files/Articulo/".$ruta)){
				...
				if(empty($_POST["txtIdArticulo"])){
					...
					if($objArticulo->Registrar($idcategoria, $idunidad_medida, $nombre, $descripcion, $marca, $ruta_img)){
						echo "Articulo Registrado";
					}else{
						echo "Articulo no ha podido ser registado.";
					}
					...
					$idarticulo = $_POST["txtIdArticulo"];
					if($objArticulo->Modificar($idarticulo, $idcategoria, $idunidad_medida, $nombre, $marca, $descripcion, $ruta_img)){
						echo "Informacion del Articulo ha sido actualizada";
					}else{
						echo "Informacion del Articulo no ha podido ser actualizada.";
					}
		...
		case "list":
            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
				if(isset($reg->Marca) && !empty($reg->Marca))
					$marca = $reg->Marca;
				else
					$marca = 'N/A';
     			...
     			$data[] = array("id"=>$i,
					...
					'<button class="btn btn-warning" data-toggle="tooltip" title="Editar" onclick="cargarDataArticulo('.$reg->idarticulo.',\''.$reg->idcategoria.'\',\''.$reg->idunidad_medida.'\',\''.$reg->nombre.'\',\''.$reg->descripcion. '\',\'' . $marca . '\',\''.$reg->imagen.'\')"><i class="fa fa-pencil"></i> </button>&nbsp;'.
