<?php
	...
	class Ingreso{
		...
		public function Listar($idsucursal){
			global $conexion;
			$sql = "select i.idingreso, i.idusuario,i.idsucursal,i.idproveedor, i.tipo_comprobante,i.serie_comprobante,i.num_comprobante, DATE_FORMAT(i.fecha, '%d / %m / %Y') as fecha,i.impuesto,i.total,i.estado, p.nombre as proveedor from ingreso i inner join persona p on i.idproveedor = p.idpersona where i.idsucursal = $idsucursal order by idingreso desc limit 0,2999";
	...
	inner join articulo a on di.idarticulo = a.idarticulo where di.idingreso = $idingreso";
		...
		public function GetProveedorSucursalIngreso($idingreso){
			global $conexion;
			$sql = "select p.*, DATE_FORMAT(ped.fecha, '%d / %m / %Y') as fecha,ped.tipo_comprobante, ped.num_comprobante, ped.serie_comprobante, s.razon_social, s.tipo_documento as documento_sucursal, s.num_documento as num_sucursal, s.direccion, s.telefono as telefono_suc,
