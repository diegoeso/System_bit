<?php
	...
	class Pedido{
		...
		public function Listar($idsucursal){
			global $conexion;
			$sql = "select p.idpedido,p.idcliente,p.idusuario,p.idsucursal,p.tipo_pedido,DATE_FORMAT (p.fecha, '%d / %m / %Y') as fecha,p.numero,p.estado, c.nombre as Cliente, c.email from pedido p inner join persona c on p.idcliente = c.idpersona where p.idsucursal = $idsucursal and c.tipo_persona = 'Cliente' and p.tipo_pedido = 'Venta' order by idpedido desc limit 0,2999";
	...
	where dp.idpedido = $idpedido";
		...
		public function ListarTipoPedidoPedido($idsucursal){
			global $conexion;
			$sql = "select p.idpedido,p.idcliente,p.idusuario,p.idsucursal,p.tipo_pedido, DATE_FORMAT(p.fecha, '%d / %m / %Y') as fecha,p.numero,p.estado,c.nombre as Cliente, c.email from pedido p inner join persona c
		...
		public function ListarDetalleIngresos($idsucursal){
			global $conexion;
			$sql = "select di.iddetalle_ingreso, di.stock_actual, a.nombre as Articulo, di.codigo, di.serie, di.precio_ventapublico, a.imagen, DATE_FORMAT(i.fecha, '%d / %m / %Y') as fecha, a.marca as Marca from ingreso i inner join detalle_ingreso di on di.idingreso = i.idingreso 	inner join articulo a on di.idarticulo = a.idarticulo 	where i.estado = 'A' and i.idsucursal = $idsucursal and di.stock_actual > 0 order by i.fecha desc";
			$query = $conexion->query($sql);
			return $query;
		}
	...
	where dds.idsucursal = $idsucursal and operacion = 'Comprobante'";
		...
		public function GetClienteSucursalPedido($idpedido){
			global $conexion;
			$sql = "select p.*, DATE_FORMAT(ped.fecha, '%d / %m / %Y') as fecha, s.razon_social, ped.numero, s.tipo_documento, s.num_documento as num_sucursal, s.direccion, s.telefono as telefono_suc, s.email as email_suc, s.representante, s.logo, ped.tipo_pedido,p.tipo_documento as doc
	...
	where ped.idpedido = $idpedido";
		...
		public function GetVenta($idpedido){
			global $conexion;
			$sql = "select p.*, DATE_FORMAT(ped.fecha, '%d / %m / %Y') as fecha, s.razon_social, v.num_comprobante, v.serie_comprobante, s.tipo_documento, s.num_documento as num_sucursal, s.direccion, s.telefono as telefono_suc, s.email as email_suc, s.representante, s.logo, ped.tipo_pedido,v.impuesto,p.tipo_documento as doc
	from persona p inner join pedido ped on ped.idcliente = p.idpersona
	inner join sucursal s on ped.idsucursal = s.idsucursal
	...
	where ped.idpedido = $idpedido";
		...
		public function ImprimirDetallePedido($idpedido){
			global $conexion;
			$sql = "select di.codigo,di.serie, a.nombre as articulo, a.marca, dp.*, (dp.cantidad * dp.precio_venta) - dp.descuento as sub_total
	...
	inner join articulo a on di.idarticulo = a.idarticulo where p.idpedido = $idpedido";//para agregar el campo marca lo puse aquii 
			$query = $conexion->query($sql);
			return $query;
		}
