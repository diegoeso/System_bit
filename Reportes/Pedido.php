...
class PDF_Invoice extends FPDF
{
	...
	function addSociete($nom, $adresse, $logo, $ext_logo)
	{
		...
		$y1 = 7;
		if (isset($ext_logo)) {
			$this->Image($logo, 10, 6, 25, 25, $ext_logo);
		}
		$this->SetXY($x1, $y1);
	...
	function fact_dev($libelle, $num)
	{
		...
		while ($loop == 0) {
			$this->SetFont("Arial", "B", $szfont); //CUADRO DE PROFORMA 
	...
	function addClient($ref)
	{
		...
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 3);
		$this->SetFont("Arial", "", 10);
		$this->Cell(10, 5, "CLIENTE", 0, 0, "C"); //CLIENT ESTABA
		$this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 9);
		$this->SetFont("Arial", "", 10);
	...
	function addPageNumber( $page )
	{
		...
		$this->Line( $r1, $mid, $r2, $mid);
		$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1+3 );
		$this->SetFont( "Arial", "", 10);//para poner negritas las textos B
	...
	function addClientAdresse ($cliente, $domicilio, $num_documento, $email, $telefono)
	{
		$r1 = $this->w - 206; //200
		$r2 = $r1 + 196; //190
		$y1 = 40;//40 mover el rectangulo 
		$y2 = $y1 + 15;//18 mover el la altura de rectangulo
		$mid = $y1 + (($y2 - $y1) / 2);
		$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2 - $y1), 1.5, 'D'); //2.5
		$this->SetXY($r1, $y1);
		$this->SetFont("Arial", "B", 10);
		$this->MultiCell(60, 6, "Cliente:"); //cliente de reporte 
		$this->SetXY($r1, $y1 + 5);
		$this->SetFont("Arial", "", 10);
		$this->MultiCell(140, 4, $cliente . ' | ' . $num_documento . ' | ' . $domicilio . ' | ' . $email . ' | ' . $telefono . ' | ');//100
	}
	...
	function addCols( $tab )
	{
		...
		$r1  = 10; //10 el cuadro grande codigo ,descripcion
		$r2  = $this->w - ($r1 * 2) ;//2
		$y1  = 58;//60 para mover la letra del descripcion del articulo
		$y2  = $this->h - 36 - $y1; //45 la altura del rectangulo
		$this->SetXY( $r1, $y1 );
		$this->Rect( $r1, $y1, $r2, $y2, "D");
		$this->Line( $r1, $y1+10, $r1+$r2, $y1+10);//mover el cuadrado 
		$colX = $r1;
		$colonnes = $tab;
		while ( list( $lib, $pos ) = each ($tab) )
		{
			$this->SetXY( $colX, $y1);
			$this->MultiCell($pos, 5, $lib, 0, "C");//4
			$colX += $pos;
	...
	*/
	...
	{
		$this->SetFont( "Arial", "B", 10); // para mover el footer 
		$r1  = 10;//10
		$r2  = $r1 + 120;//120
		$y1  = $this->h - 32;//40 pa mover el cuadro
		$y2  = $y1+20;//20 controla el ancho de la altura Son
		...
		$this->Cell(10,4, "Son:");//4    10
		$this->SetFont( "Arial", "B", 8);
		$this->SetXY( $r1+9, $y1+7);//7
		$this->MultiCell(100,4, $monto);//100
		//$this->SetX( $r1+29 );
	...
	function addTVAs( $igv, $total,$moneda )
	{
		...
		$re  = $this->w - 30;//30 para mover el ancho de las letras totales
		$rf  = $this->w - 29;//29
		$y1  = $this->h - 32; //20 bajar el rectangulo total
		$r1 = $this->w - 70;//70 para mover el rectangulo ancho
		$r2 = $r1 + 60;// el ancho del rectangulo total
		$y2 = $y1 + 20;//20

		$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2 - $y1), 1.5, 'D');
		$this->SetFont( "Arial", "B", 10);//8
		$this->SetXY( $re, $y1+5 );
		$this->Cell( 17,4, "Total Neto:  " .$moneda.sprintf("%0.2F", $total), '', '', 'R'); //17AGREGANDO total
