...
{
	$x1 = 35; //para mover el logo y letras
	$y1 = 10;
	//Positionnement en bas

	$this->Image($logo , 10,5, 25 , 30, $ext_logo);//utilizar para mover el logo
	$this->SetXY( $x1, $y1 );
	...
	$this->MultiCell($length, 4, $adresse); //para mover el espacio de los datos de la empresa 
}
...
function fact_dev( $libelle, $num )
{
    ...
    while ( $loop == 0 )
    {
       $this->SetFont( "Arial", "B", $szfont );//Cuadro de Recibo
    ...
    $this->SetFillColor(255,255,255); //PARA LOS COLORES DEL CONTORNO DEL RECIBO
    $this->RoundedRect($r1, $y1, ($r2 - $r1), $y2, 2.5, 'DF'); //PARA HACER LINEAS REDONDAS
...
{
	$r1     = $this->w - 206; //editar donde se queda la pocision de los nombres
	$r2     = $r1 + 195;
	$y1     = 40; 
	$y2 = $y1 + 16;//15 mover el la altura del rectangulo
	$mid = $y1 + (($y2 - $y1) / 2);
	$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2 - $y1), 1.5, 'D'); //2.5
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "B", 10);
	$this->MultiCell( 60, 4, "Señor(es):");// colocar en el sistema
	$this->SetXY( $r1, $y1 + 5);
	$this->SetFont( "Arial", "", 10);
...
function addCols( $tab )
{
	...
	$r1  = 10;//10 LAS CLUMNAS CODIGO DESCRIPCION
	$r2  = $this->w - ($r1 * 2) ;
	$y1  = 58; //58 para mover el rectangulo descripcion del articulo codigo
	$y2  = $this->h - 35 - $y1; //36 cacular la altura del rectangulo
	$this->SetXY( $r1, $y1 );
...
function addCadreTVAs($monto)
{
	...
	$this->Cell(10,4, "SON:"); // modificacion del report 
	$this->SetFont( "Arial", "B", 8);
	$this->SetXY( $r1+9, $y1+7);
	$this->MultiCell(100,4, $monto);
	// te sirve para coloca medidas de seguridad 
	//$this->Cell(, , "Gracias por su compra JAMER insumos electricos.");
	//$this->SetX( $r1+29 );
...
{
	$r1  = $this->w - 60;//52 ESTO AL LADO DERECHO
	$r2  = $r1 + 50;//43 MOVER LA ANCHURA IZQUIERO
	$y1  = $this->h - 32; //MUEVE LA LETRA TOTAL A PAGAR 
	$y2  = $y1+20; //LA ALTURA DEL RECTANGULO
	$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2-$y1), 2.5, 'D');
	$this->Line( $r1+20,  $y1, $r1+20, $y2); // LA LINEA DEL CENTRO DEL RECTANGULO
	//$this->Line( $r1+20, $y1+4, $r2, $y1+4); // Sous Euros & Francs
	//$this->Line( $r1+38,  $y1, $r1+38, $y2); // Entre Euros & Francs
	$this->SetFont( "Arial", "B", 10);
	$this->SetXY( $r1+22, $y1 );
	//$this->Cell(15,4, "TOTALES", 0, 0, "C");HACER ESTO EN EL SISTEMA
	$this->SetFont( "Arial", "B", 8);
	//$this->SetXY( $r1+42, $y1 );
...
function addTVAs( $igv, $total,$moneda )
{
	...
	$re  = $this->w - 35; //Con esto movi el texto
	$rf  = $this->w - 10;
	$y1  = $this->h - 31;
	$this->SetFont( "Arial", "B", 8);
	$this->SetXY( $re, $y1+5 );
